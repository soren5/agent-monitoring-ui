...
describe('requestApproval delivery failure', () => {
  ...
  it('persists a caller-supplied host deadline for a bounded approval', async () => {
    ...
    await requestApproval({
      ...
      payload: { key: 'value' },
      title: 'Deadline',
      question: 'Approve before expiry?',
      expiresAt: deadline,
    });
    expect(getPendingApprovalsByAction('deadline_action')).toMatchObject([{ expires_at: deadline, status: 'pending' }]);
  });
});
