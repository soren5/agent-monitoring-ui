...
export function materializeTemplateSkills(agentGroupId: string, destSkillsDir: string): void {
  ...
  for (const name of fs.readdirSync(src)) {
    // lstat (not stat) so dangling container-path symlinks (e.g.
    // agent-browser → /app/skills/agent-browser) are skipped instead of
    // throwing ENOENT — they're provider-shared links, not template skills.
    let isDir = false;
    try {
      isDir = fs.lstatSync(path.join(src, name)).isDirectory();
    } catch {
      continue;
    }
    if (!isDir) continue;
