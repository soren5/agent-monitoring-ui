...
async function deliverMessage(
  ...
  // Provider-migration handoff: when a migration is awaiting the agent's
  // ready marker, consume it here before any channel delivery — the operator
  // never sees the marker in chat. Same host-owned seam as the smoke test.
  if (hasTable(getDb(), 'provider_migrations')) {
    const { observeProviderHandoff } = await import('./modules/agent-to-agent/provider-migration.js');
    if (observeProviderHandoff(session.agent_group_id, msg.content)) return;
  }
  ...
  if (msg.channel_type === 'agent') {
    ...
    const { observeProvisionedSmokeResponse } = await import('./modules/agent-to-agent/provisioning.js');
    if (observeProvisionedSmokeResponse(session.agent_group_id, msg.platform_id, msg.content)) return;
