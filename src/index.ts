...
import { startCodexAuthSync, stopCodexAuthSync } from './onecli-codex-sync.js';
...
async function main(): Promise<void> {
  ...
  // 6b. Sync refreshed host codex auth into the OneCLI vault so the gateway
  // stops serving expired tokens. Best-effort — the module logs its own failures.
  startCodexAuthSync();
  ...
  try {
    await startUsageReaderBridge();
  } catch (err) {
    log.error('Usage reader bridge did not start', { err });
  }
