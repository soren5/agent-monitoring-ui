...
export async function routeInbound(event: InboundEvent): Promise<void> {
  ...
  // A host-owned project channel has exactly one ordinary wiring: its parent.
  // An exact `@alias` (or an already-bound thread) selects a child without
  // adding that child to the messaging-group fan-out table.  We retain the
  // parent wiring's sender and thread policies, but substitute only the
  // delivery principal.  Thus an unaddressed message still reaches one parent
  // exactly once, while a child can never receive ambient channel traffic.
  // Keep this dynamic: the project module depends on approval delivery,
  // which registers router interceptors during module initialization. A
  // static router → project → approvals → router import would enter the
  // router module before its interceptor registry exists.
  const { resolveProjectRecipient } = await import('./modules/agent-to-agent/projects.js');
  const projectRecipient = resolveProjectRecipient(mg.id, messageText, event.threadId ?? null);
  const deliveryAgents: MessagingGroupAgent[] = projectRecipient
    ? agents.slice(0, 1).map((agent) => ({ ...agent, agent_group_id: projectRecipient }))
    : agents;
