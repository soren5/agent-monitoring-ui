 ...
 *
 * agents.create is now a retired compatibility action. Generic descendants
 * are created only through the immutable owner-approved provisioning
 * contract; no CLI scope, including `global`, can turn a raw container system
 * message into a new central-DB principal.
 *
...
export const agentsCreate = defineGuardedAction({
  ...
  grantCoversRequest: (grant, input) => {
    try {
      const approved = JSON.parse(grant.payload) as { name?: string; provider?: string | null; model?: string | null };
      return (
        approved.name === input.payload.name &&
        (approved.provider ?? null) === (input.payload.provider ?? null) &&
        (approved.model ?? null) === (input.payload.model ?? null)
      );
    } catch {
      return false;
    }
  ...
  decide: (input) => {
    ...
    return DENY(
      'Legacy create_agent is disabled; use factory_request_agent_provision for an owner-approved constrained child.',
    );
  },
});
