...
function parseCreateAgentRequest(content: Record<string, unknown>): CreateAgentRequest | null {
  const name = typeof content.name === 'string' ? content.name : '';
  const instructions = typeof content.instructions === 'string' ? content.instructions : null;
  const provider = content.provider === 'openai-compatible' ? 'openai-compatible' : null;
  const model = typeof content.model === 'string' ? content.model : null;
  if (!name) return null;
  if (content.provider != null && !provider) return null;
  if ((provider && (!model || !LOCAL_MODELS.has(model))) || (!provider && model)) return null;
  return { name, instructions, provider, model };
}
...
export function validateCreateAgent(content: Record<string, unknown>, session: Session): boolean {
  const request = parseCreateAgentRequest(content);
  if (!request) {
    notifyAgent(
      session,
      'create_agent failed: provide a name; local agents require provider=openai-compatible and an approved model.',
    );
    return false;
  }
...
export async function requestCreateAgentHold(content: Record<string, unknown>, session: Session): Promise<void> {
  ...
  await requestApproval({
    ...
    action: 'create_agent',
    payload: { ...request },
    title: `Create agent: ${request.name}`,
    question: `Agent "${sourceGroup.name}" wants to create "${request.name}"${request.provider ? ` using local model ${request.model}` : ''}. Approve?`,
  });
}
...
export async function createAgent(content: Record<string, unknown>, session: Session): Promise<void> {
  const request = parseCreateAgentRequest(content);
  const sourceGroup = getAgentGroup(session.agent_group_id);
  if (!request || !sourceGroup) return; // precheck already answered the requester

  await performCreateAgent(request, session, sourceGroup, (text) => notifyAgent(session, text));
}
...
async function performCreateAgent(
  ...
  const localName = normalizeName(request.name);

  // Collision in the creator's destination namespace
  if (getDestinationByName(sourceGroup.id, localName)) {
    notify(`Cannot create agent "${request.name}": you already have a destination named "${localName}".`);
    return;
  }
  ...
  if (!resolvedPath.startsWith(resolvedGroupsDir + path.sep)) {
    notify(`Cannot create agent "${request.name}": invalid folder path.`);
  ...
  initGroupFilesystem(newGroup, { instructions: request.instructions ?? undefined, provider });
  if (request.provider && request.model)
    updateContainerConfigScalars(agentGroupId, { model: request.model, cli_scope: 'disabled' });
