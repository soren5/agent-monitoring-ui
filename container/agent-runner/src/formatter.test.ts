...
describe('agent dispatch messages', () => {
  it('renders plain-text parent dispatches instead of dropping them', () => {
    getInboundDb()
      .prepare(`INSERT INTO messages_in (id, kind, timestamp, status, content) VALUES (?, ?, ?, 'pending', ?)`)
      .run('dispatch-1', 'agent', new Date().toISOString(), 'create the allowed branch');
    const result = formatMessages(getPendingMessages());
    expect(result).toContain('<agent_message');
    expect(result).toContain('create the allowed branch');
  });
});
