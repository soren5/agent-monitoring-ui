  ...
  async function messageToInbound(
    ...
    if (message.attachments && message.attachments.length > 0) {
      ...
      for (const att of message.attachments) {
        ...
        // Chat SDK adapters expose fetchData() to retrieve attachment bytes.
        // The Discord adapter omits it but provides the CDN `url` — fall back
        // to fetching that so attached files reach the agent (otherwise the
        // agent only sees attachment metadata, never the bytes).
        ...
        if (!buffer && typeof att.url === 'string' && att.url) {
          try {
        ...
        const card = Card({
          ...
          children: [
            ...
            Actions(
              ...
              options.map((opt, idx) => {
                // Approval decisions are security-sensitive and their values
                // are short fixed strings. Encode them directly so a Discord
                // click never depends on a later index-to-option lookup.
                // Generic questions retain compact index encoding for
                // Telegram's callback-data limit.
                const actionValue = questionId.startsWith('appr-') ? opt.value : String(idx);
                return Button({
                  id: `ncq:${questionId}:${actionValue}`,
                  label: opt.label,
                  value: actionValue,
                  style: opt.style,
                });
...
async function handleForwardedEvent(
  ...
  if (event.type === 'GATEWAY_INTERACTION_CREATE' && event.data) {
    ...
    if (interaction.type === 3) {
      ...
      // Discord's Chat SDK encodes a button as `<action id>\n<value>`.
      // Our action id already contains the question and selected value, so
      // discard the SDK-appended value before parsing it. Without this,
      // "approve" arrived as "approve\napprove" and was treated as a reject.
      const actionId = customId?.split('\n', 1)[0];

      // Parse the selected option from the action id.
