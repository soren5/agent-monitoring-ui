...
import { USAGE_READER_BRIDGE_URL } from '../usage-reader-bridge.js';
...
registerProviderContainerConfig(
  ...
  (ctx) => {
    ...
    // This is a fixed action endpoint, not a host shell. The runner can only
    // request a usage snapshot and the host bridge always invokes one exact,
    // separately Accessibility-authorized helper binary.
    return { mounts, env: { NANOCLAW_USAGE_READER_BRIDGE_URL: USAGE_READER_BRIDGE_URL } };
  },
