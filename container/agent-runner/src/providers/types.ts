...
export interface ProviderExchange {
  ...
  /**
   * Thinking-mode output for the exchange (opencode `reasoning` parts).
   * Optional — persisted to the conversation archive so reasoning is
   * observable, never shown in channel delivery.
   */
  reasoning?: string;
}
...
export type ProviderEvent =
  ...
  /**
   * Token usage for a completed turn. Optional — providers with an in-band
   * usage source (e.g. the OpenAI-compatible DeepSeek endpoint) emit one
   * aggregated event per turn right before their result; others never do.
   */
 ...
 * single figure the operator report surfaces (the true cost of solving the
 * problem). The prompt/completion/reasoning split is captured for internal
 * audit only — it is persisted, never shown in the Discord report.
 */
export interface ProviderUsage {
  promptTokens: number;
  completionTokens: number;
  /** Thinking-mode output tokens (DeepSeek `completion_tokens_details.reasoning_tokens`). */
