...
async function sweep(): Promise<void> {
  ...
  try {
    const { sweepExpiredProvisionApprovals } = await import('./modules/agent-to-agent/provisioning.js');
    await sweepExpiredProvisionApprovals();
  } catch (err) {
    log.error('Provisioning approval expiry sweep failed', { err });
  }

  try {
    const { sweepProvisionedSmokeTests } = await import('./modules/agent-to-agent/provisioning.js');
    sweepProvisionedSmokeTests();
  } catch (err) {
    log.error('Provisioned smoke-test sweep failed', { err });
  }

  try {
    const { sweepExpiredProviderMigrations } = await import('./modules/agent-to-agent/provider-migration.js');
    sweepExpiredProviderMigrations();
  } catch (err) {
    log.error('Provider-migration sweep failed', { err });
  }
