...
import { findDiscordRouting, isCorruptionError, processQuery } from './poll-loop.js';
...
describe('formatter', () => {
  it('selects the Discord route from a mixed batch for usage reporting', () => {
    const routing = findDiscordRouting([
      {
        id: 'task-1', kind: 'task', timestamp: 0, status: 'pending', process_after: null,
        trigger: 1, on_wake: 0, content: '{}', platform_id: null, channel_type: null, thread_id: null,
      },
      {
        id: 'discord-1', kind: 'chat-sdk', timestamp: 0, status: 'pending', process_after: null,
        trigger: 1, on_wake: 0, content: '{}', platform_id: 'discord-channel', channel_type: 'discord', thread_id: 'thread-1',
      },
    ]);

    expect(routing).toEqual({
      platformId: 'discord-channel', channelType: 'discord', threadId: 'thread-1', inReplyTo: 'discord-1', taskRun: false,
    });
  });
...
describe('deepseek usage reporting through processQuery', () => {
  it('emits a Discord usage report when the stream carries a usage event', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'poll-deepseek-usage-'));
    const { setDeepseekBalanceFetcherForTest, startDeepseekUsageJob } = await import('./deepseek-usage-job.js');
    setDeepseekBalanceFetcherForTest(async () => ({ currency: 'USD', total_balance: '9.99' }));

    const routing = { platformId: 'chan-1', channelType: 'discord', threadId: null, inReplyTo: 'm1', taskRun: false };
    const usageJob = startDeepseekUsageJob({ providerName: 'deepseek', cwd: tmpDir, routing });

    async function* events(): AsyncGenerator<ProviderEvent> {
      yield { type: 'init', continuation: 'sess-deepseek' };
      yield {
        type: 'usage',
        usage: { promptTokens: 100, completionTokens: 567, reasoningTokens: 400, totalTokens: 667 },
      };
      yield { type: 'result', text: '<message to="chan-1">all done</message>' };
    }
    const query: AgentQuery = {
      push: () => {},
      end: () => {},
    ...
    await processQuery(query, routing, ['m1'], 'deepseek', undefined, 'prompt', undefined, {
      cwd: tmpDir,
      initialUsageJob: usageJob ? { provider: 'deepseek', job: usageJob } : null,
    });

    const out = getUndeliveredMessages();
    const usageReports = out
      .map((m) => JSON.parse(m.content) as { text?: string })
      .filter((c) => c.text?.startsWith('DeepSeek usage for'));
    expect(usageReports).toHaveLength(1);
    expect(usageReports[0].text).toContain('total tokens: 667');
    expect(usageReports[0].text).toContain('DeepSeek balance remaining: USD 9.99');
    expect(usageReports[0].text).toContain('Codex weekly limit remaining: unavailable');

    setDeepseekBalanceFetcherForTest(null);
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it('emits no usage report for a stream without a usage event', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'poll-deepseek-usage-none-'));
    async function* events(): AsyncGenerator<ProviderEvent> {
      yield { type: 'init', continuation: 'sess-deepseek' };
      yield { type: 'result', text: '<message to="chan-1">plain answer</message>' };
    }
    const query: AgentQuery = {
      push: () => {},
      end: () => {},
