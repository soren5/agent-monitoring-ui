...
export function formatMessages(messages: MessageInRow[]): string {
  ...
  if (agentMessages.length > 0) {
    parts.push(...agentMessages.map(formatAgentMessage));
  }
...
/** Host-dispatched/a2a messages are plain text, not channel JSON envelopes. */
function formatAgentMessage(msg: MessageInRow): string {
  const from = originAttr(msg);
  const time = formatLocalTime(msg.timestamp, TIMEZONE);
  return `<agent_message${from} time="${escapeXml(time)}">${escapeXml(msg.content)}</agent_message>`;
}
