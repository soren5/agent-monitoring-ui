...
/** Shared in-process adapter for providers that speak OpenAI function calling. */
export function registeredTools(): McpToolDefinition[] {
  return [...allTools];
}

export async function callRegisteredTool(name: string, args: Record<string, unknown>) {
  const tool = toolMap.get(name);
  if (!tool) return { content: [{ type: 'text' as const, text: `Unknown tool: ${name}` }] };
