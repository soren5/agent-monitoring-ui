...
import { EGRESS_NETWORK, egressNetworkArgs, ensureEgressNetwork, ensureLocalModelBridge } from './egress-lockdown.js';
...
async function buildContainerArgs(
  ...
  //
  // The internal egress network is ONLY for reaching the local model bridge
  // (LM Studio on the host) — it has no outbound internet and no host-gateway
  // route, by design. Providers/models that need real network egress (deepseek
  // via the OneCLI gateway) must use the default bridge host-gateway path
  // instead. opencode uses the egress network only when it drives a LOCAL
  // model (gemma); opencode + deepseek rides the host gateway like native
  // deepseek does.
  const providerIsOpenaiCompat = _provider === 'openai-compatible';
  const providerIsOpencode = _provider === 'opencode';
  const harnessUsesLocalModel =
    providerIsOpenaiCompat || (providerIsOpencode && !(containerConfig.model ?? '').startsWith('deepseek'));
  if (ensureEgressNetwork() || harnessUsesLocalModel) {
    if (harnessUsesLocalModel) ensureLocalModelBridge();
...
export async function buildAgentGroupImage(agentGroupId: string): Promise<void> {
  ...
  try {
    ...
    await execFileAsync(CONTAINER_RUNTIME_BIN, ['build', '-t', imageTag, '-f', tmpDockerfile, '.'], {
      // Docker needs the project root as its build context. Running from
      // DATA_DIR makes the generated `.` context omit the repository (and
      // can fail outright when the service has a different working dir).
